...
TEST(BinarySearch, ShouldSearchCorrectIndex)
{
    ...
    const size_t ActualIndex = Algo::BinarySearch(SortedArray.Data(), SortedArray.Size(), TargetValue);
...
TEST(BinarySearch, ShouldSearchCorrectIndexWithProjection)
{
    ...
    const size_t ActualIndex2 = Algo::BinarySearch(SortedArray.Data(), SortedArray.Size(), TargetValue, [](const int& Value) { return Value * 2; }, Less<int>());
...
TEST(BinarySearch, ShouldSearchOtherTypes)
{
    ...
    const size_t ActualCharIndex = Algo::BinarySearch(SortedCharArray.Data(), SortedCharArray.Size(), TargetChar);
